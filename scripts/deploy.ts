/**
 * Deploy ConfidentialPiggyBank to Ethereum Sepolia.
 *
 * This is a plain ethers v6 script (run with tsx) rather than a Hardhat-native
 * task, so the deploy path has as few moving parts as possible. It reads the
 * ABI + bytecode from the artifact produced by `npx hardhat compile`.
 *
 * Env vars (see .env.example):
 *   SEPOLIA_RPC_URL  - an Ethereum Sepolia JSON-RPC endpoint
 *   PRIVATE_KEY      - 0x-prefixed key of a funded Sepolia dev wallet
 */
...
/** Recursively find ConfidentialPiggyBank.json under artifacts/. */
function findArtifact(dir: string): string | null {
  ...
  for (const entry of entries) {
    ...
    } else if (entry === "ConfidentialPiggyBank.json") {
      return full;
    }
  }
  return null;
...
async function main() {
  ...
  if (!artifactPath) {
    throw new Error(
      "Could not find a compiled artifact for ConfidentialPiggyBank.\n" +
  ...
  console.log("=====================================================");
  console.log("");
  console.log("Next: put this address in your .env as CONTRACT_ADDRESS, then run:");
  console.log("  npm run roundtrip:sepolia");
